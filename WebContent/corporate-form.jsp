<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<html>
<head>
<title>Corporate Management Application</title>
<link rel="stylesheet"
	href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
	integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T"
	crossorigin="anonymous">
</head>
<body>

	<header>
		<nav class="navbar navbar-expand-md navbar-dark"
			style="background-color: blue">
			<div>
				<a href="https://www.xadmin.net" class="navbar-brand"> Corporate
					Management Application </a>
			</div>

			<ul class="navbar-nav">
				<li><a href="<%=request.getContextPath()%>/list"
					class="nav-link">Corporates</a></li>
			</ul>
		</nav>
	</header>
	<br>
	<div class="container col-md-5">
		<div class="card">
			<div class="card-body">
				<c:if test="${corporate != null}">
					<form action="update" method="post">
				</c:if>
				<c:if test="${corporate == null}">
					<form action="insert" method="post">
				</c:if>

				<caption>
					<h2>
						<c:if test="${corporate != null}">
            			Edit Corporate
            		</c:if>
						<c:if test="${corporate == null}">
            			Add New Corporate
            		</c:if>
					</h2>
				</caption>

				<c:if test="${corporate != null}">
					<input type="hidden" name="id" value="<c:out value='${corporate.id}' />" />
				</c:if>

				<fieldset class="form-group">
					<label>Corporate Name</label> <input type="text"
						value="<c:out value='${corporate.name}' />" class="form-control"
						name="name" required="required">
				</fieldset>

				<fieldset class="form-group">
					<label>Account Number</label> <input type="text"
						value="<c:out value='${corporate.email}' />" class="form-control"
						name="accountNumber">
				</fieldset>

				<button type="submit" class="btn btn-success">Save</button>
				</form>
			</div>
		</div>
	</div>
</body>
</html>